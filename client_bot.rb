...
# Global hash to store conversation state per chat
$client_states = {}

# Helper: if the message has a photo caption, use editMessageCaption; otherwise editMessageText.
...
def handle_message(bot, message)
  ...
  when SUBSCRIPTION_CLIENT
    ...
    # Default handler: show the main menu
    keyboard = Telegram::Bot::Types::InlineKeyboardMarkup.new(inline_keyboard: [
      [ Telegram::Bot::Types::InlineKeyboardButton.new(text: "عرض المشاكل", callback_data: "menu_show_tickets") ]
    ])
...
def handle_callback_query(bot, callback_query)
  ...
  when /^ignore\|(\d+)/
    ...
    if ["Client Responded", "Client Ignored", "Closed"].include?(ticket["status"])
      ...
      # Notify supervisors about the client response
...
def send_issue_details_to_client(bot, message, ticket_id)
  ticket = DB.get_ticket(ticket_id)
...
def send_full_issue_details_to_client(bot, message, ticket_id)
  ticket = DB.get_ticket(ticket_id)
...
def notify_supervisors_client_response(ticket_id, opts = {})
  ticket = DB.get_ticket(ticket_id)
